...
public class FwdByDist extends DriveAbstract {
    ...
    public void initialize() {
        ...
        double deltaX = distance * Math.cos(currentHeading);  // ← Horizontal
        double deltaY = distance * Math.sin(currentHeading);  // ← Vertical

        // Create target pose by adding forward vector to current position
        targetPose = new Pose(
                currentPose.getX() + deltaX,
                currentPose.getY() + deltaY,
                drive.toPedroHeading(currentHeading)  // Maintain heading
        );
